...
if [[ ${V_GUI_SEL} == "awesome" ]]; then
    export V_GUI="xfce4-terminal lightdm lightdm-gtk-greeter awesome feh gnu-free-fonts slock terminus-font ttf-liberation xorg-server xorg-xinit xorg-xrandr xsel xterm"
    export V_SERVICES=("lightdm.service ${V_SERVICES_MY}")

# bspwm
elif [[ ${V_GUI_SEL} == "bspwm" ]]; then
    export V_GUI="xfce4-terminal lightdm lightdm-gtk-greeter bspwm dmenu rxvt-unicode sxhkd xdo"
    export V_SERVICES=("lightdm.service ${V_SERVICES_MY}")

# budgie
elif [[ ${V_GUI_SEL} == "budgie" ]]; then
    export V_GUI="xfce4-terminal lightdm lightdm-gtk-greeter budgie mate-terminal materia-gtk-theme nemo papirus-icon-theme"
    export V_SERVICES=("gdm.service ${V_SERVICES_MY}")

# cinnamon
elif [[ ${V_GUI_SEL} == "cinnamon" ]]; then
    export V_GUI="blueman bluez-utils cinnamon cinnamon-translations engrampa gnome-keyring gnome-screenshot xfce4-terminal gvfs-smb system-config-printer xdg-user-dirs-gtk xed lightdm lightdm-gtk-greeter"
    export V_SERVICES=("lightdm.service ${V_SERVICES_MY}")

# cosmic
elif [[ ${V_GUI_SEL} == "cosmic" ]]; then
    export V_GUI="cosmic-session cosmic-text-editor cosmic-files cosmic-terminal cosmic-player cosmic-wallpapers xfce4-terminal gvfs-smb system-config-printer xdg-user-dirs-gtk xed cosmic-greeter"
...
# i3
    ...
    export V_GUI="xfce4-terminal dmenu i3-wm i3blocks i3lock i3status xss-lock xterm lightdm lightdm-gtk-greeter"
    export V_SERVICES=("lightdm.service ${V_SERVICES_MY}")

# kde
elif [[ ${V_GUI_SEL} == "kde" ]]; then
    export V_GUI="ark dolphin kate xfce4-terminal plasma plasma-workspace kde-applications sddm"
...
# mate
    ...
    export V_GUI="xfce4-terminal lightdm lightdm-gtk-greeter mate mate-extra"
    export V_SERVICES=("lightdm.service ${V_SERVICES_MY}")

# qtile
elif [[ ${V_GUI_SEL} == "qtile" ]]; then
    export V_GUI="xfce4-terminal lightdm lightdm-gtk-greeter qtile"
    export V_SERVICES=("lightdm.service ${V_SERVICES_MY}")

# sway
elif [[ ${V_GUI_SEL} == "sway" ]]; then
    export V_GUI="xfce4-terminal lightdm lightdm-gtk-greeter brightnessctl foot grim pavucontrol slurp sway swaybg swayidle swaylock waybar wmenu xorg-xwayland"
    export V_SERVICES=("lightdm.service ${V_SERVICES_MY}")

# xfce
elif [[ ${V_GUI_SEL} == "xfce" ]]; then
    export V_GUI="gvfs xarchiver xfce4-terminal xfce4 xfce4-goodies xfce4-screenshooter xfce4-screensaver xfce4-power-manager system-config-printer pavucontrol xfce4-places-plugin xfce4-mixer gnome-keyring lightdm lightdm-gtk-greeter"
    export V_SERVICES=("lightdm.service ${V_SERVICES_MY}")

# xmonad
elif [[ ${V_GUI_SEL} == "xmonad" ]]; then
    export V_GUI="xfce4-terminal lightdm lightdm-gtk-greeter dmenu xmonad xmonad-contrib xmonad-extra xterm"
    export V_SERVICES=("lightdm.service ${V_SERVICES_MY}")
...
#=> Backup profile:
    ...
    arch-chroot /mnt bash -c "chown -R ${V_USER_NAME} /home/${V_USER_NAME}/backup-archinstall-profile" 2> /dev/null || error_log "Backup archinstall script to User dir. (chown dir)"
    arch-chroot /mnt bash -c "exit" || error_log "Not EXIT chroot problem."
